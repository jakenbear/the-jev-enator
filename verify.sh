...
for spec in "PreToolUse:$GATE:danger gate" "PostToolUse:$NOTICE:failure notice" "PostToolUseFailure:$NOTICE:failure notice (failed commands)" "Stop:$FINISH:completion check" "PreToolUse:$SCOPE:scope check" "PreToolUse:$READS:reads ranker" "UserPromptSubmit:$CLEAR:clear advisor"; do
...
# 5a. The scope check has no enforcing mode at all. Stated here rather than left
# implicit: a reader who sees a fourth hook in settings.json should be told, by the
# tool, that it cannot block anything.
if [[ "${JEV_SCOPE_OFF:-}" == "1" ]]; then
  note "JEV_SCOPE_OFF=1 — the scope check is off (the other hooks still run)"
else
...
if [[ "${JEV_CLEAR_OFF:-}" == "1" ]]; then
  note "JEV_CLEAR_OFF=1 — the clear advisor is off (the other hooks still run)"
else
  ok "clear advisor notes you (only you) when a prompt starts a new task over 60k"
fi
...
for hook, label in (("gate", "danger gate"), ("notice", "failure notice"), ("finish", "completion check"), ("scope", "scope check"), ("reads", "reads ranker"), ("clear", "clear advisor")):
